...
void function () {
  ...
  vidius.TexturePacker = function () {
    ...
    TexturePacker.prototype.add = function (image) {
      ...
      if (null != node) {
        ...
        return new vidius.Sprite(node.left, node.top, node.width, node.height);
      } else {
  ...
  vidius.DisplayList = function () {
    function DisplayList(param$, virtualWidth, virtualHeight, textureSize, overlay) {
      ...
      this.canvas = param$;
      this.texturePacker = new vidius.TexturePacker(textureSize);
      this.sprites = [];
      displayList = this;
      glsl = Glsl({
        canvas: this.canvas,
